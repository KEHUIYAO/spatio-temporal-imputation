...
class PM25_Dataset(Dataset):
    """PM2.5 dataset for training, validation, and test.
    Args:
        eval_length (int): length of observed data
        target_dim (int): length of target data
        mode (str): train, valid, test
        validindex (int): index of validation month
    ...
    def __init__(self, eval_length=36, target_dim=36, mode="train", validindex=0):
        ...
        with open(path, "rb") as f:
            self.train_mean, self.train_std = pickle.load(f)  # mean and std of training data
        if mode == "train":
            ...
            month_list = [1, 2, 4, 5, 7, 8, 10, 11]
            month_list = month_list[validindex : validindex + 1]  #‘validindex’th month is used for validation
        elif mode == "test":
            month_list = [3, 6, 9, 12]
    ...
    def __getitem__(self, org_index):
        ...
        s = {
            ...
            "for_pattern_mask": self.observed_mask[hist_month][
